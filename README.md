...
2. 同行地图：玩家定位光点与分身视觉跟随、高德真实底图、WGS-84 到 GCJ-02 坐标转换、训练地图降级。
3. 共同决策：Coze 生成短观察；失败时明确使用同人格预置结果。
4. 记忆闭环：前台运动/GPS 计步、25 米双定位自动拾取、建议、玩家确认、打卡、虚拟物品、储物柜式娃屋。
...
真实同行只在页面前台运行。运动传感器不可用时会明确降级为 GPS 距离估算；Web Demo 不读取系统健康步数，也不承诺页面关闭后的后台定位。
